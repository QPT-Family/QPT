...
* QPT的缺陷
  - [ ] 由于是“模拟”开发环境，所以相较传统打包对项目的规范程度有一定要求，越规范的项目越不容易踩坑，但也提升了使用成本。
  - [ ] 依赖虽然不会都被打包进去，但若您没有Requirements文件，那么在QPT自动生成Requirements文件后需要确认是否正确，否则可能会有依赖漏掉的情况。
  - [ ] 在您搭建项目时所踩下的坑QPT并不会完全打包进去，极端情况下仍需要您手动撰写SubModule来保证用户可以正常使用。
