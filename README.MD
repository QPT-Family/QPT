...
* 使用QPT的优势
  ```
  【可定制兼容方案】在打包形如PaddlePaddle深度学习Python库时也可流畅打包，让QPT来捕捉你的习惯。  
...
* QPT的缺陷  
  ...
  【依赖处理】依赖虽然只会打包源码中出现的依赖包，但如果该依赖包不规范（大部分Python包是规范的）则可能会出现依赖缺失的问题。若您没有Requirements文件，那么在QPT自动生成Requirements文件后需要您确认依赖是否完备，否则可能会有依赖漏掉的情况。  
  【踩坑继承】在您搭建项目时所踩下的坑QPT在模拟时可能并不会打包进去，极端情况下仍需要您手动撰写SubModule来保证用户可以正常使用。 
  ``` 
